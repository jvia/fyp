...
public abstract class AbstractAucomGraph implements SourceStatusListener, SinkStatusListener, Serializable, TransformNodeEventListener {
	...
	/**
	 * fts Graph construction function. It is expected that after calling this function the topology of the fts graph used by this implementation of the AbstractAucomGraph is established. 
	 */
	...
	/**
	 * Save a dot representation of the fts graph used by this object. Does nothing in case the fts graph is not constructed yet.
	 */
	...
	/**
	 * Utility function. Offers a convenient mean to check whether the status of this graph object is {@link #GraphStatus NOTREADY}
	 * @return
	 */
	...
	/**
	 * Utility function. Offers a convenient mean to check whether the status of this graph object is {@link #GraphStatus RUNNING}
	 * @return
	 */
	...
	/**
	 * Starts the graph. Does nothing in case the graph is already running. Otherwise checks whether preconditions are satisfied and starts the fts graph
	 * @throws ActionFailedException When 1) the graph status is {@link GraphStatus STOPPED} 2) preconditions are not satisfied i.e., {@link #preconditionsSatisfied()} returns false  3) an exception has been caught while starting the fts graph. 
	 */
	public void start() throws ActionFailedException {
		...
		try {
			...
			Iterator<Node<?, ?>> it = graph.getSourceNodes().iterator();
			while (it.hasNext()) {
				AucomSourceAdapter<?> source = (AucomSourceAdapter<?>) it.next();
				source.connect();
			}
	...
	/**
	 * Indicates whether the preconditions for the immediate execution of the graph are met. Postcondition: If this function returns True the status of the graph must not be {@link #GraphStatus NOTREADY}  
	 */
	...
	/**
	 * Resumes graph execution by resuming the execution of all sources of the fts graph if the graph was previously paused. Otherwise the call to this function has no effect.
	 */
	public void resume() {
		if (currentStatus.equals(GraphStatus.PAUSED)) {
			resumeAllSources();
	...
	/**
	 * Stop this Graph if {@link GraphStatus} is not STOPPED. Afterwards this graph object cannot be restarted again In doing so the {@link #cleanUp()} function will be called which offers a customized way to neatly clean up any resources used by this graph 
	 */
	public void stop() {
	...
	/**
	 * Interface offering a mean to clean up resources used by this graph before stopping this graph. Called by {@link #stop()}
	 */
	protected abstract void cleanUp();

	/**
	 * SourceEvent handling function. Changes {@link #currentStatus} to RUNNING if a source notifies about sending its first element ({@link #SourceStatus FIRST_ELEMENT_SENT}) and if {@link #currentStatus} is not RUNNING 
	 */
	@Override
	...
	/**
	 * TransformNodeEvent handling function. Changes {@link #currentStatus} to READY if a transformation node notifies about receiving last element ({@link #NodeStatus RECEIVEDLASTELEMENT}) and if {@link #currentStatus} is RUNNING 
	 */
	@Override
	...
	/**
	 * SinkStatusEvent handling function. Changes {@link #currentStatus} to READY if a sink node notifies about receiving last element ({@link #NodeStatus RECEIVEDLASTELEMENT}) and if {@link #currentStatus} is RUNNING 
	 */
	@Override
	...
	/**
	 * Sets the state of the graph. It additionally save the previous state to {@link previousStatus} and Notifies all {@link GraphStatusListener}
	 * 
