...
public class TrainOptimizeClassificateExperiment implements Experiment {
	...
	private ClassificatorOptimizer optimizer;
	private File modelOutputFile;

	public TrainOptimizeClassificateExperiment(File inTrainingFile, File inOptimizingFile, File inTestingFile) {
		...
		trainer.addModelTrainerListener(new ModelTrainerListener() {
			...
			public void modelTrainerStatusChanged(StatusChangedEvent evt) {
				if (evt.getPreviousStatus().equals(TrainerStatus.RUNNING) && evt.getCurrentStatus().equals(TrainerStatus.READY)) {
					...
					Logger.getLogger(TrainOptimizeClassificateExperiment.this.getClass().getCanonicalName()).info("model trained ...");
				}
			}
		});
