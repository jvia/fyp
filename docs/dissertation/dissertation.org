...
#+begin_latex
  ...
  \begin{abstract}
    ...
    have relied on explicit modeling of component behavior,
    quantitatively and qualitatively, but these techniques do not scale
    to complex robots operating in dynamic environments. A new technique
    which involves making the robot self-aware to the internal state of
    its own components is examined. This project implements and then
    measures the efficacy of a probabilistic self-awareness model for
    event-based robotics middlewares. Alternative model formulations are
    proposed and examined experimentally and asymptotically to deal with
