...
distributions becomes the model $\mathcal{M} = \{P_{ij}|(e_i,e_j) \in E \times
E\}$. $\mathcal{M}$ is now the matrix shown in \eqref{matrix:ex1}.
...
with properties to push the algorithm (and changes made to it) in some
way. What follows now is a brief description of the experimental
setups which will be reference in the rest of the dissertation.
...
of the model must grow quadratically.
