...
\noindent where $E$ is the set of last seen instance of each event type and
$w_{ij}$ is the relative weighting of the probability value. The
weight $w_{ij}$ 

$h_{ij}$ is the entropy of the distribution of $P_{ij}$, and $$H_j =
\sum^E_{e_i} h_{ij}$$


#\left(1 - \frac{h_{ij}}{H_j}\right)

In equation \ref{eq:score}, it can be seen that the score is
calculated over
...
#+begin_src gnuplot :var data="./data/reduced_3chain_fault" :exports none :file "img/reduced_3chain_fault.eps" 
  reset
...
#+begin_src gnuplot :var data="./data/reduced_4x4_normal.csv" :exports none :file "img/reduced_4x4_normal.eps" 
  reset
  set terminal postscript color solid eps enhanced 20
  set yrange [0:1]
  set xrange [0:150000]
...
#+begin_src gnuplot :var data="./data/reduced_4x4_fault.csv" :exports none :file "img/reduced_4x4_fault.eps" 
  reset
  set terminal postscript color solid eps enhanced 20
  set yrange [0:1]
  set xrange [0:150000]
...
#+begin_src gnuplot :var data="./data/reduced_10x1_fault.csv" :exports results :file "img/reduced_10x1_fault.eps" 
  reset
  set terminal postscript color solid eps enhanced 20
  set yrange [0:1]
  set xrange [0:150000]
