...
#+LATEX_HEADER: \graphicspath{{./diagrams}{./images}}
#+LATEX_HEADER: \DeclareGraphicsExtensions{.eps,.ps,.pdf,.png}
...
node /A/ publishes a message /a/ which is passes to node /B/. Node
/B/, after doing some arbitrary computation, publishes a message /b/
which is passed to node /C/. So, in this example, it can be expected
that data flows linearly from node /A/ to node /C/. It is not
...
communication patterns. Figure \ref{fig:ex2] shows a system in which
node /A/ publishes two types of messages, each of which is used by a
separate component.
...
figures \ref{fig:ex1} and \ref{fig:ex2} as examples. Fist, the idea
